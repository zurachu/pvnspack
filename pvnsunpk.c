...
unsigned char* ppack_findPackDataEx(const char *fpkName, const char *fName)
{
	...
	// ファイルの先頭1セクタのポインタを取得
	if(_FileOpen(&pfa, fpkName, FOMD_RD)) { return NULL; }
	...
	if(i >= fpi.famount) { return NULL; }	// 発見失敗
	
	if(ext == 1 && strstr(fName, ".pvn") != NULL) { fi.size++; }	// pvnファイル処理
	if((ret = pceHeapAlloc(fi.size)) == NULL) { return NULL; }	// メモリ確保
